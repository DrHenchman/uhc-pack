execute as @e[tag=pre_generator] run execute if score @s uhcPGQueue = LoadTime uhcPG run function uhc_pack:pre_generation/stage/step/chunk/unload
execute as @e[tag=pre_generator] run execute if score @s uhcPGQueue < LoadTime uhcPG run scoreboard players add @s uhcPGQueue 1
